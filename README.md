...
保存され、次回起動時も続きから使えます。既知の旧版（v1.0.0〜v1.3.1）を保存して
いる場合は、確認後にシステム領域だけを最新版へ更新し、利用者のファイルを保持します。
識別できない版や将来版は上書きしません。
...
`vendor/`にはsuperzazu/z80とymfmのビルドに必要なソースを同梱しています。
新規にcheckoutしたワークツリーから、そのまま次のコマンドを実行できます。
